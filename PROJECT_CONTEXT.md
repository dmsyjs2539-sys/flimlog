...
`main.js`의 `initCursorTrail()`. 마우스가 지나간 자리에 물방울 같은 입자가 남았다 사라진다.

- 전체화면 `<canvas class="cursor_trail">`를 JS로 생성. `position: fixed`, `z-index: 90`(헤더 아래),
  `pointer-events: none`이라 클릭을 가로채지 않는다.
- 입자는 방사형 그라디언트 원. 수명 동안 살짝 부풀며 흐려지고 아주 천천히 아래로 흘러내린다.
- 색은 앰버 `#ffb900` / 테라코타 `#efae90` / 크림 `#f6f5f2` / 딥그린 `#0d3322` 4종.
  밝은 크림 배경과 어두운 히어로 영상 어느 쪽에서도 최소 두 가지가 보이도록 섞었다.
- 커서가 빠를수록 프레임당 입자를 1 → 2 → 3개로 늘려 꼬리가 길어진다. 최대 160개로 상한.
- **입자가 모두 사라지면 `requestAnimationFrame`을 멈춘다.** 유휴 상태에서 CPU를 쓰지 않는다.
- `(hover: hover) and (pointer: fine)`이 아니거나 `prefers-reduced-motion: reduce`면 캔버스를
  아예 만들지 않는다. 도중에 모션 최소화로 바뀌면 즉시 비운다.
- DPR은 2로 상한을 둬 고해상도 디스플레이에서 버퍼가 과도해지지 않게 했다.
